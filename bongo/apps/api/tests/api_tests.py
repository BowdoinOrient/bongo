...
def crud(self, object, model, endpoint=None):
    ...
    self.assertEqual(res.status_code, 200)
    self.assertIn(object.pk, [v for k, v in json.loads(res.content.decode("utf-8"))['body'][0].items() if k=='id'])
    ...
    try:
        ...
        if res.status_code == 400 and 'body' in json.loads(res.content.decode("utf-8")) and json.loads(res.content.decode("utf-8")).get('body').get('staticfile') == [u'This field is required.']:
